...
class AgeWallet_Admin {
	 ...
	 * @var string
	 */
	private $page_slug = 'agewallet-settings';
	...
	public function add_admin_menu() {
		...
		// Add the "Settings" sub-menu page.
		add_submenu_page(
			$this->page_slug,
	...
	public function register_settings() {
		...
		// New Phase 3/4 Settings.
		register_setting( $this->option_group, 'agewallet_protection_mode', array( 'sanitize_callback' => 'sanitize_text_field', 'default' => 'standard' ) );
		register_setting( $this->option_group, 'agewallet_gate_page_id', array( 'sanitize_callback' => 'absint', 'default' => 0 ) );
		register_setting( $this->option_group, 'agewallet_excluded_paths', array( 'sanitize_callback' => array( $this, 'sanitize_paths_textarea' ), 'default' => '' ) );
		...
		// Fields - Guarding.
		add_settings_field( 'agewallet_protection_mode', __( 'Security Mode', 'agewallet' ), array( $this, 'render_protection_mode_radio' ), $this->page_slug, 'agewallet_oidc_section_guarding' );
		add_settings_field( AgeWalletOIDCClientPro::OPT_BLOCK_MODE, __( 'Scope of Protection', 'agewallet' ), array( $this, 'render_radio_buttons' ), $this->page_slug, 'agewallet_oidc_section_guarding', array( 'option_name' => AgeWalletOIDCClientPro::OPT_BLOCK_MODE, 'options' => array( 'none' => __( 'No automatic protection.', 'agewallet' ), 'all_but_home' => __( 'Protect entire site, except homepage.', 'agewallet' ), 'all' => __( 'Protect entire site, including homepage.', 'agewallet' ), 'specific' => __( 'Protect only specific URL paths.', 'agewallet' ) ) ) );
		...
		// New Excluded Paths Field
		add_settings_field( 'agewallet_excluded_paths', __( 'Paths to Exclude', 'agewallet' ), array( $this, 'render_simple_textarea' ), $this->page_slug, 'agewallet_oidc_section_guarding', array( 'label_for' => 'agewallet_excluded_paths', 'class' => 'large-text', 'rows' => 3, 'placeholder' => '/privacy-policy/, /contact/', 'desc' => __( 'Exceptions to Global Protection. Any URL containing these paths will be visible.', 'agewallet' ) ) );

		// New Gate Page Selector
		add_settings_field( 'agewallet_gate_page_id', __( 'Age Gate Page', 'agewallet' ), array( $this, 'render_gate_page_dropdown' ), $this->page_slug, 'agewallet_oidc_section_guarding' );
	...
	/**
	 * Renders the Protection Mode radio buttons.
	 * @since 1.1.0
	 */
		...
		<fieldset>
			...
			<div>
				<label>
					...
					<strong><?php esc_html_e( 'High Security (Redirect)', 'agewallet' ); ?></strong> -
					<span class="description"><?php esc_html_e( 'Redirects unverified users to the Age Gate Page. Content is never sent to browser until verified. Compatible with "Cache Everything".', 'agewallet' ); ?></span>
				</label>
				<br><small style="margin-left: 25px; color: #666;"><?php esc_html_e( 'Note: High Security mode disables the [agewallet_protected] shortcode functionality. The entire page will be protected instead.', 'agewallet' ); ?></small>
			</div>
		</fieldset>
		<?php
	}
	...
	/**
	 * Renders the Age Gate Page dropdown selector.
	 * @since 1.1.0
	 */
	public function render_gate_page_dropdown() {
		$option_name = 'agewallet_gate_page_id';
		$val         = get_option( $option_name, 0 );

		wp_dropdown_pages(
			array(
				'name'              => $option_name,
				'show_option_none'  => __( '&mdash; Select a Page &mdash;', 'agewallet' ),
				'option_none_value' => '0',
				'selected'          => $val,
			)
		);
		?>
		<p class="description">
			<?php esc_html_e( 'Select the page where unverified users will be redirected in "High Security" mode. This page should contain your age verification button or instructions.', 'agewallet' ); ?>
		</p>
		<?php
	}

	/**
	 * Renders a textarea field specifically for blocked paths (with JS toggle).
	 * @since 0.1.0
	 */
	public function render_textarea( $args ) {
		...
		$style_attr   = ( 'specific' !== $current_mode ) ? 'style="display:none;"' : '';
	...
	/**
	 * Renders a simple textarea field (always visible).
	 * @since 1.1.0
	 */
